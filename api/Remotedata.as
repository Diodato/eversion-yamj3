...
class api.Remotedata {
	...
	function Remotedata() {
		this.fn = {harddataloaded:Delegate.create(this, this.harddataloaded),
				   ...
				   onYAMJ3genresIndexData:Delegate.create(this, this.onYAMJ3genresIndexData),
				   harddataloaded1:Delegate.create(this, this.harddataloaded1)
				  };
	}
			...
			if(data[0].file=="YAMJ3") {
				...
				switch(data[0]["originaltitle"]) {
					...
					default:
						...
						this.Callback("ERROR", "Unsupported index: "+data[0]["originaltitle"]);
				}
			}
			else
				harddataloaded1(status,message,data)
		} else {
			trace(status+" : " + message);
			this.Callback(status, message);
		}
...
//			if(data.length>20) {
				for(var i=0;i<this.totalTitles;i++) {
					...
					// add it
					addto.push( {action:"SWITCH",
							   data:"Genres_"+data[i]["name"]+"_1",
							   file:"Genres_"+data[i]["name"]+"_1",
							   title:data[i]["name"],
							   originaltitle:data[i]["name"]});
...
// ****************** PREFETCH **********************
	...
	private function prefetch_load(thisPage:Number) {
		...
		if(this.pageCount[thisPage] == null) {
			...
			// load it
			this.loading=true;
			this.datasource.getData(thisPage,this.fn.add_page_data);
		}
	}

...
class ev.EVMain {
	...
	public function create(parentMC:MovieClip):Void {
		...
		this.fn = {	settingsDone:Delegate.create(this, this.settingsDone),
					...
					yamj3checkDone:Delegate.create(this, this.yamj3checkDone),
	...
	private function skinsettingsDone(status, message) {
		...
		if(status!="ERROR") {
			...
			// and we're off
//			this.startskin();
			this.mainMC.message_txt.text="Checking for YAMJ3";
			this.yamj3.checkForYAMJ3(this.fn.yamj3checkDone);
		} else {
			trace("error "+message);
			this.mainMC.message_txt.text="ERROR: "+message;
		}
	...
	private function yamj3checkDone(success:Boolean) {
		if(success)
		{
			trace("YAMJ3 is running");
			Common.yamj3isrunning=true;
		}
		else
		{
			trace("YAMJ3 is not running");

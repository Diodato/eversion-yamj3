...
// ************************** PLAYBACK ********************************
	...
	private function do_playfile(data) {
		...
		if(data.raw.files[0].fileName!=undefined || data.raw.files[0].fileName!=null)
			Mediaplayer.addFromRaw(data.raw, data.raw.files[0].fileName, this.fn.playerdone, this.parentMC);
		else
...
//		trace("playnum: "+part);
		var filename:String=playinfo.raw.filename.fileName;
...
//				Mediaplayer.addEP(playinfo.epraw, 0, 0, this.fn.playerdone, this.parentMC);
				Mediaplayer.addFromRaw(playinfo.raw, filename, this.fn.playerdone, this.parentMC);
				break;
			case 'PLAYPART':
			case 'PLAYSINGLE':
...
//				Mediaplayer.addEP(playinfo.epraw, part, part, this.fn.playerdone, this.parentMC);
	...
	private function start_playback(playinfo:Object) {
		...
		var filename:String=playinfo.raw.files[0].fileName;
//		var part=0;
...
// ************************** ALERTS *********************************
	private function alert_check() {
		...
		if(Background.verevbadupdate==true) {  // check beta2 upgraded wrong
			...
			Background.verjb=false;
		} else if(Background.yamj3check==true) { // check if YAMJ3 is running
			trace(".. YAMJ3 not running alert detected");
			this.start_popup("POPUP", "alert", Common.evPrompts.enoyamj3);
			Background.yamj3check=false;
		}
	}
